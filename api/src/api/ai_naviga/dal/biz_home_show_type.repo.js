...
const model = require('../model/biz_home_show_type.model');
const factory = require('../factory');

/**
 * biz_home_show_type — 核心表
 */
class BizHomeShowTypeRepo extends Dal {
  constructor() {
    super();
    this.modelType = model;
    this.tableName = 'biz_home_show_type';
    this.defalutOrder = 'biz_home_show_type.sort_index asc';
    this.tableTitle = '页面显示类型';
    this.primaryKey = 'id';
    ...
    this.baseSql = `
      select biz_home_show_type.*
      from biz_home_show_type biz_home_show_type
      where biz_home_show_type.id > 0 {0} {1}
      order by {2}
    `.replace(/\n\s+/g, ' ').trim();
  }
  ...
  GetSearchSQL({ searchModel, userId }) {
    ...
    if (searchModel.Keyword && searchModel.Keyword.trim()) {
      w.w('biz_home_show_type.title', searchModel.Keyword, 'like', 'or');
    }
    if (searchModel.id) w.eq('biz_home_show_type.id', searchModel.id);
    if (searchModel.ids) w.in('biz_home_show_type.id', searchModel.ids);
    return w.build();
  }
}

module.exports = new BizHomeShowTypeRepo();